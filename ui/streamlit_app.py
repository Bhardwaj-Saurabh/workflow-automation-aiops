"""
Streamlit UI for AI Assessment Evaluation System.

This provides an interactive web interface for:
- Uploading documents
...
For beginners: This shows how to build a complete web app using just Python!
"""
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        **Features:**
        ...
        # Statistics (if workflow has run)
        if st.session_state.workflow_result:
            ...
            if result.get("assessment"):
                ...
                st.metric("Total Questions", len(assessment.questions))
                st.metric("Overall Score", f"{assessment.percentage:.1f}%")
                
                if result.get("questions_needing_review"):
                    st.metric("Needs Review", len(result["questions_needing_review"]))
        ...
        with st.spinner("Processing document..."):
            try:
                ...
                if uploaded_file:
                    ...
                    result = workflow.run(document_path=tmp_path)
    ...
    for i, q in enumerate(questions, 1):
        with st.container():
            ...
            <div class="question-card">
                <h4>Question {i} ({q.question_type.value})</h4>
                <p><strong>Q:</strong> {q.text}</p>
                <p><strong>Student Answer:</strong> {q.user_answer}</p>
                {f'<p><strong>Expected:</strong> {q.reference_answer}</p>' if q.reference_answer else ''}
...
def page_evaluation():
    ...
    if not evaluations:
        st.error("No evaluations available")
        return
    ...
    if assessment:
        ...
        with col4:
            ...
            st.metric("Correct", f"{correct_count}/{len(evaluations)}")
...
def page_human_review():
    ...
    questions_needing_review = result.get("questions_needing_review", [])
    ...
    evaluations = {e.question_id: e for e in result["evaluations"]}
    ...
    if st.button("✅ Submit Reviews", type="primary", use_container_width=True):
        # Update workflow result with human feedback
        result["human_feedback"] = st.session_state.human_feedback
        
        # Re-run finalize and report generation
        with st.spinner("Finalizing scores..."):
            workflow = AssessmentWorkflow()
            # Apply human feedback and regenerate
